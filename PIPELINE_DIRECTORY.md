...
| **Textures** | ✅ Phases A–E all done; Phase F (multi-tile) next | 31+ sets; 5 biome kits all purpose-built and bound to per-mode .tres (walk/iso/topdown); `wgv3_rock_dark` has 2K/1K/512 mip ladder | `pipelines/textures/aaa_texture.py` (canonical; `--ladder` for hero-quality) |
