# Attach to the Decal node from `blood_spray_large_decal.tscn` to enable
# animated UV playback. For static decals (logos, blood splats),
# the .tscn alone (frame 0) is enough.
extends Decal
class_name VFXDecalFlipbook_blood_spray_large

const SHADER_MATERIAL := preload("res://vfx/spell/blood_spray_large/godot/decal_material.tres")
