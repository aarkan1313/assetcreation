# Attach to the Decal node from `dust_puff_small_decal.tscn` to enable
# animated UV playback. For static decals (logos, blood splats),
# the .tscn alone (frame 0) is enough.
extends Decal
class_name VFXDecalFlipbook_dust_puff_small

const SHADER_MATERIAL := preload("res://vfx/spell/dust_puff_small/godot/decal_material.tres")
