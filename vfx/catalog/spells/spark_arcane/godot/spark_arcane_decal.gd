# Attach to the Decal node from `spark_arcane_decal.tscn` to enable
# animated UV playback. For static decals (logos, blood splats),
# the .tscn alone (frame 0) is enough.
extends Decal
class_name VFXDecalFlipbook_spark_arcane

const SHADER_MATERIAL := preload("res://vfx/spell/spark_arcane/godot/decal_material.tres")
