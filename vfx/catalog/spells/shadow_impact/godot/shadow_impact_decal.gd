# Attach to the Decal node from `shadow_impact_decal.tscn` to enable
# animated UV playback. For static decals (logos, blood splats),
# the .tscn alone (frame 0) is enough.
extends Decal
class_name VFXDecalFlipbook_shadow_impact

const SHADER_MATERIAL := preload("res://vfx/spell/shadow_impact/godot/decal_material.tres")
