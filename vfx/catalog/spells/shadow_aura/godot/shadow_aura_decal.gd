# Attach to the Decal node from `shadow_aura_decal.tscn` to enable
# animated UV playback. For static decals (logos, blood splats),
# the .tscn alone (frame 0) is enough.
extends Decal
class_name VFXDecalFlipbook_shadow_aura

const SHADER_MATERIAL := preload("res://vfx/ambient/shadow_aura/godot/decal_material.tres")
