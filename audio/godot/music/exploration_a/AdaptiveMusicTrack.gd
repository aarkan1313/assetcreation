...
extends Node
class_name AdaptiveMusicTrack_exploration_a

const STEMS: Array = [{"id": "pad", "intensity_min": 0.0, "intensity_max": 1.0}, {"id": "harp", "intensity_min": 0.2, "intensity_max": 1.0}, {"id": "low_strings", "intensity_min": 0.5, "intensity_max": 1.0}]
const STATES: Dictionary = {"open": {"intensity": 0.4}, "discovery": {"intensity": 0.8}}
const BPM: float = 90
const BARS: int = 32
...
func _build_synchronized_stream() -> AudioStreamSynchronized:
	...
	for i in range(STEMS.size()):
		...
		var stem_path := "res://audio/music/exploration_a/{stem}.wav".format({{"stem": info.id}})
