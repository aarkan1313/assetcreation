...
extends Node
class_name AdaptiveMusicTrack_bossfight_a

const STEMS: Array = [{"id": "drums", "intensity_min": 0.0, "intensity_max": 1.0}, {"id": "low_brass", "intensity_min": 0.0, "intensity_max": 1.0}, {"id": "choir", "intensity_min": 0.3, "intensity_max": 1.0}, {"id": "lead_strings", "intensity_min": 0.5, "intensity_max": 1.0}, {"id": "perc_taiko", "intensity_min": 0.8, "intensity_max": 1.0}]
const STATES: Dictionary = {"intro": {"intensity": 0.5}, "phase_1": {"intensity": 0.7}, "phase_2": {"intensity": 0.9}, "phase_3": {"intensity": 1.0}, "defeated": {"intensity": 0.4, "next_track": "victory_a"}}
const BPM: float = 140
const BARS: int = 16
...
func _build_synchronized_stream() -> AudioStreamSynchronized:
	...
	for i in range(STEMS.size()):
		...
		var stem_path := "res://audio/music/bossfight_a/{stem}.wav".format({{"stem": info.id}})
