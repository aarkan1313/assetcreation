...
extends Node
class_name AdaptiveMusicTrack_town_a

const STEMS: Array = [{"id": "guitar", "intensity_min": 0.0, "intensity_max": 1.0}, {"id": "flute", "intensity_min": 0.3, "intensity_max": 1.0}, {"id": "tambourine", "intensity_min": 0.6, "intensity_max": 1.0}]
const STATES: Dictionary = {"morning": {"intensity": 0.5}, "evening": {"intensity": 0.8}, "festival": {"intensity": 1.0}}
const BPM: float = 100
const BARS: int = 24
...
func _build_synchronized_stream() -> AudioStreamSynchronized:
	...
	for i in range(STEMS.size()):
		...
		var stem_path := "res://audio/music/town_a/{stem}.wav".format({{"stem": info.id}})
