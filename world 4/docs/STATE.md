...
> **What we have right now.** Last updated: 2026-05-12.
>
...
- 🚧 **Stage 3 — Heightmap stack integration**:
  - ✅ 3.1 — ClipmapRing displacement-texture slot.
  - ⏳ 3.2 → 3.5 — pending. Next is the heightmap-sampling shader
    (`terrain_world_v3.gdshader`), then `ClipmapWorld` wires to
    `KernelComposer` (sync first), then `WorkerThreadPool` async,
    then `HeightMapShape3D` collision proxy.
...
| `scale_v2/` (4 km × 4 km, clipmap rings) | **in progress** (Axis 1 Path 2) | Has `material_clipmap_debug.tres` for Stage 2 captures. Heightmap stack (Stage 3) not yet wired. |
...
- **ClipmapWorld + nested rings** (`scripts/ClipmapWorld.gd` +
  `terrain_world_v3.gdshader` — v3 in progress): the new Axis 1 Path
  2 stack. 4 ring meshes that translate with the camera. Kernel-driven
  heightmaps via `KernelComposer`. **The 4 km scale_v2 will run on
  this.**
...
| Clipmap renderer | `scripts/ClipmapWorld.gd`, `scripts/ClipmapRing.gd`, `shaders/clipmap_debug.gdshader` (+ `terrain_world_v3.gdshader` WIP) | Stage 2+3 work. Donut meshes, camera-snap, displacement textures. |
...
fix per entry. Current count: **6 documented pitfall classes** (black
speckles, scan-line, PBR-at-scale, fingerprint bands, Texture2DArray
gotchas ×2, per-tile splat boundary discontinuity). Plus session-12
additions: shader `VERTEX.y = h` clobbers skirts (use `+=`),
`inner_grid_n` rounding direction matters for ring overlap.
