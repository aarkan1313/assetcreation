...
**Status:** **Phase 1 pipeline mechanically correct, FLUX prompt quality is the open gap (2026-05-07 night).** Full GPU pipeline runs without errors: Blender 5.1 headless → FLUX.1-Fill-dev-fp8 (prompt-only by default) → nvdiffrast back-projection → GLB repack. Five bugs diagnosed and fixed after visual inspection of intermediates: (1) camera orbit started at 0° (side view) → fixed to 270° (front); (2) mask cy at 66% down (hip) → 45% (sternum); (3) Redux is style adapter not logo compositor → `use_redux=False` default with prompt-only inpaint; (4) FLUX result was pasted raw (transparent BG included) → fixed to composite only inside mask AND inside character alpha; (5) back-projector azimuth was 0-based while renderer is 270-based → fixed to match. Output `goblin_p_ashen_v5.glb` produced. **Remaining gap:** FLUX.1-Fill fills mask with "more goblin skin" — surrounding context overwhelms text prompt. Next: prompt engineering (higher denoise, explicit emblem shape desc) or ControlNet-reference approach to force the emblem to appear.
