...
> ⚠️ **Canonical roadmap is [`ROADMAP.md`](ROADMAP.md).** This doc is
> the orchestrator state inventory (worker boundaries, knob-space,
> existing infrastructure pointers) — not the authority on the active
> milestone or M-chain ordering. If state language here disagrees with
> ROADMAP.md or the latest committed audit, ROADMAP.md wins.
>
> The 2026-05-11 audit reset the active milestone to M19; see
> [`WORLD3_M1_M18_AUDIT_REPORT_2026_05_11.md`](WORLD3_M1_M18_AUDIT_REPORT_2026_05_11.md)
> and ROADMAP.md for the canonical current state. Historical
> "M14 active" / "M18 closure" language below remains as decision-trail
> context.
...
Repo-level data inventory now lives in `docs/MASTER_DATA_CATALOG.md` with
machine-readable `world3/data_catalog.json`, regenerated by
`pipelines/terrain/build_master_catalog.py`. Use that catalog as the authority
for what DEMs, master stacks, real-source classes, texture sets, and coverage
gaps exist before requesting more data or selecting procedural references.

OpenTopo/DEM data is reference/training/runtime source data, not something to
stretch into infinity by repeating one crop. The long-term procedural direction
is to extract landform/material/drainage/vegetation rules from many cataloged
sources, stream or mosaic compatible neighbors when available, and generate
believable extensions from those learned rules.

2026-05-09 methodology correction: same-source repeated tiling is diagnostic
only. The production-facing path is terrain seam integration: a generated
world-space band that solves height, normals, material weights, source macro
...
- **2026-05-08 (M5 pass 1)**: Wired `walk.tscn` to visible 256 m
  ...
  height sampler's source fraction. Added `M5WalkStreamRunner.gd` plus
...
- **2026-05-08 (M7-M12 planning)**: Added `M7_M12_NEAR_ROADMAP.md` to lock the
  ...
  organic source cleanup, runtime polish, terrain seam integration plus
  cross-source blending, junctions, and walk/iso/topdown parity. Deferred
  scatter/props/buildings/fantasy remain after that lane.
...
- **2026-05-09 (auto/full-map review correction)**: Removed true far/horizon
  views from the default tour after mobile review exposed finite-footprint
  limits. Then fixed source-height repeat at the runtime level by making
  `ChunkLoader.gd` default to mirrored finite-source sampling. Added
  `source_stack_full_map_fast_tour.tscn` for faster full-footprint movement and
...
- **2026-05-10 (M14 temperate forest floor bakeoff)**: Added
  ...
  `source_stack_gloss_temperate_forest_humus_v3_tour.tscn` and passed live
  review as conditional M14 sidecar evidence. It is not production-promoted.
