...
## Parallel: Orchestration — Phase E (replaces former O1-O3)

The "O1-O3" items from `WORLD3_OPERABILITY_GAPS_2026_05_10.md` were
absorbed into the Phase B-E rebuild plan and **shipped 2026-05-11 as
E.1-E.5**. Pipeline throughput is no longer the bottleneck for M19.

What landed (read the per-phase docs for detail):

- **E.1** — `region_request_schema.json` + 3 examples + validator
  ([E1_E3_ORCHESTRATOR_MVP_2026_05_11.md](E1_E3_ORCHESTRATOR_MVP_2026_05_11.md))
- **E.2** — `stages.json` manifest + audit script (12 stages, all clean)
- **E.3** — `world3_make.py` orchestrator (byte-identical reproduction of
  canonical bundles from a config file)
- **E.4** — Parameterized `OrchestratorCaptureDriver.gd` + `.tscn` +
  `run_orchestrator_capture.py` wrapper; one driver replaces N hand-authored
  capture scenes ([E4_ORCHESTRATOR_CAPTURE_DRIVER_2026_05_11.md](E4_ORCHESTRATOR_CAPTURE_DRIVER_2026_05_11.md)
  has the M18 cascade validation and the M10 procedural smoke)
- **E.5** — Style pack mechanism: `world3/jobs/style_packs/<id>.json` schema,
  `photoreal` default reproduces existing look byte-equivalent
  ([E5_STYLE_PACK_MECHANISM_2026_05_11.md](E5_STYLE_PACK_MECHANISM_2026_05_11.md))

What's still open in Phase E:

- **E.6** — Docs + handoff refresh (this section is part of it; ongoing)
- **E.7** — Final validation on a never-seen region as the integration test

Bottom line for a new orchestrator: the "I have data X, I want world type Y,
make it" command exists today as
`python world3/pipeline/world3_make.py <request.json>`.
