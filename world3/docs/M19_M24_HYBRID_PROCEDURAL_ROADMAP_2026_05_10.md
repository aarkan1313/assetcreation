...
> ⚠️ **SUPERSEDED 2026-05-11.** This doc is retained for institutional
> context only. Its scope was absorbed into the unified Phase F / G / H
> layout:
>
> - **M19** (corpus spectral analysis) → split between **F.4** (transition
>   audit) and **H.3** (full corpus-driven spectral fitting)
> - **M20** (erosion sim backbone) → **H.2**
> - **M21** (procedural neighbor streaming contract) → **F.7** (multi-bundle
>   streaming director)
> - **M22** (real-DEM patch seeding) → **G.6**
> - **M23** (infinite streaming director) → **F.7**
> - **M24** (style lane + playable infinite slice) → **E.5** (style packs
>   landed) + **F.7** (playable slice)
>
> **Canonical roadmap**: [WORLD3_LONG_ARC_2026_05_11.md](WORLD3_LONG_ARC_2026_05_11.md).
> The unification's anti-duplication ledger explains every M-number
> mapping. Do not start new work from this doc.
